...
export const useProactiveTrainingScan = () => {
  ...
  useEffect(() => {
    if (!profile || isDemoMode || !(profile.role === 'ncoic' || profile.role === 'leadership'))
      return;
    ...
    const interval = setInterval(scanTraining, 14400000); // Scan every 4 hours (Spark free-tier friendly)
    return () => {
