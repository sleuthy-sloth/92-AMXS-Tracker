...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let qPersonnel;
    if (profile.role === 'leadership') {
      qPersonnel = query(collection(db, 'users'), where('isDemo', '==', false));
    } else {
      qPersonnel = query(
        collection(db, 'users'),
        where('amuId', '==', profile.amuId),
    ...
    let qTraining;
    if (profile.role === 'leadership') {
      qTraining = query(collection(db, 'trainings'), where('isDemo', '==', false));
    } else {
      qTraining = query(
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 mt-2">
              <span className="tech-label text-primary font-bold tracking-widest text-[11px]">
                92ND AMXS
              </span>
      ...
      <div
        className="grid grid-cols-1 md:grid-cols-4 gap-0 visible-grid"
        data-tour="dashboard-matrix"
      >
        <motion.div
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-10 gap-1.5">
              {Array.from({ length: 100 }).map((_, i) => {
                ...
                const isExpiring =
                  !isActive && i < readiness + (expiringCount / totalTraining) * 100;
                return (
                  <div
                    key={i}
                    className={cn(
        ...
        >
          ...
          <p className="text-[11px] font-bold text-on-surface-variant/70 uppercase tracking-widest mt-2">
            Open Discrepancies
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="text-[11px] font-bold text-safety-orange/70 uppercase tracking-widest mt-2">
            Urgent Maintenance
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="text-[11px] font-bold text-caution-yellow/70 uppercase tracking-widest mt-2">
            Due &lt; 60 Days
          </p>
        </motion.div>
            ...
            <div className="p-8 flex justify-between items-center border-b border-outline">
              <div>
                ...
                <p className="serif-header text-sm text-slate-600">
                  Active duty personnel and qualification status
                </p>
              </div>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-on-surface-variant" />
                <input
                  className="sleek-input pl-10 py-2 text-xs w-80 !bg-background"
