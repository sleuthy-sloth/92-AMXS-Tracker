...
export const G081Gallery: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const allLogs = snapshot.docs.map(
  ...
  const handleVerify = async (logId: string) => {
    ...
    try {
      ...
      await writeAuditLog('logs', logId, 'update', {
        summary: `G081 photo verified and archived: log ${logId}`,
      });
    } catch (error) {
  ...
  const handleArchive = async (logId: string, archive: boolean) => {
    try {
      ...
      await writeAuditLog('logs', logId, archive ? 'archive' : 'restore', {
        summary: `G081 photo ${archive ? 'archived' : 'restored'}: log ${logId}`,
      });
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, 'logs');
    }
  ...
  const handleDelete = async (logId: string) => {
    ...
    try {
      ...
      await writeAuditLog('logs', logId, 'delete', { summary: 'G081 photo record deleted' });
    } catch (error) {
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <p className="serif-header text-sm text-slate-500 italic">
                Work Proof Verification & Reaction Board
              </p>
            </div>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-outline">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-5 font-black text-slate-900 uppercase tracking-tighter">
                    {log.tail_number}
                  </td>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="tech-label text-slate-400 !text-[8px] mt-1">
                          {log.technician_name} • {format(tsToDate(log.timestamp), 'MMM dd, HH:mm')}
                        </p>
                      </div>
                      <span className="px-2 py-1 bg-putty text-[9px] font-black uppercase tracking-widest">
                        {log.shopId}
                      </span>
                    </div>
                    ...
                    {log.g081_status === 'verified' ? (
                      <div className="flex-1 flex items-center gap-3 text-emerald-600">
                        ...
                        <div className="flex flex-col">
                          <span className="tech-label !text-emerald-600 uppercase">
                            Verified G081 Good
                          </span>
                          <span className="text-[9px] text-emerald-600/60 font-medium">
                            By {log.g081_verified_by}
                          </span>
                        </div>
                      </div>
                    ) : (
